#!/usr/bin/env python3
"""
🔧 PART 2: FIX FUTURE FILE GENERATION TEMPLATES
...
def example_lesson_generation_with_header():
    ...
    lesson_content = generate_your_lesson_content()  # Your existing function
...
def example_test_generation_with_header():
    ...
    test_content = generate_your_test_content()  # Your existing function
...
def example_config_generation_with_header():
    ...
    config_content = generate_your_config_content()  # Your existing function
...
BREATHING_FRAMEWORK_TEMPLATE_CONFIG = {{
    ...
    "template_integration_status": {{
        ...
        "615_test_framework_integration": True,
        "15_system_support_active": True
    }},
    ...
    def _generate_template_summary(self, results: List[Dict]) -> Dict:
        ...
        return {
            ...
            "breathing_framework_status": {
                ...
                "615_test_integration": "ACTIVE",
                "15_system_support": "READY"
            },
            "updated_templates": [str(t) for t in self.updated_templates],
            "results_detail": results,
            "next_steps": [
                f"✅ {successful} templates now auto-generate breathing framework headers" if successful > 0 else "⚠️ No templates were updated - check template locations",
                "🔄 All new generated files will automatically include proper headers" if successful > 0 else "🔍 Verify template file locations and naming conventions",
                "📝 Test template updates by generating sample files" if successful > 0 else "📁 Check if templates are in different directories",
                "✅ Validate all generated files now include headers" if successful > 0 else "🔧 Manual header addition may be needed for templates"
            ],
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print("Usage: python part2_template_generation_fix.py <base_directory>")
        print("Example: python part2_template_generation_fix.py 'C:\\Users\\Wales\\OneDrive\\Desktop\\PROGRESSIVE_FRAMEWORK-Set-2\\B2 Optimised 16_08_2025'")
        sys.exit(1)
    
    base_directory = sys.argv[1]
    
    print("🔧 BREATHING FRAMEWORK - PART 2")
